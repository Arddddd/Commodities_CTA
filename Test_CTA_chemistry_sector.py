...
def main() -> None:
    ...
    sections.append("\n## 1. 基准策略设定（lookback=10）\n")
    sections.append(
        "- 资产池：化工板块内所有品种（剔除板块指数列 `CIFI.WI`）。\n"
        "- 因子 A：`compute_vol_adj_mom`，`vol_lookback=20`，信号为动量/波动率。\n"
        "- 因子 B：`compute_t_stat_nw`，`lags=1`（HAC 修正 t-stat）。\n"
        "- 因子 C：`compute_tanh_signal`，`z_lookback=252`（raw_mom → z-score → tanh）。\n"
        "- 权重：对板块内趋势最强的品种做多top_n，对趋势最弱的品种做空bottom_n。\n"
        "- 手续费：按日换手的 0.03% 计入。\n"
    )
