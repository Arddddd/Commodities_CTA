# -*- coding: utf-8 -*-
"""Test_CTA_trend_daily_signal_v8.py

V8 = V7 + dynamic lookback selection per sector regime.

Regime logic (per sector, on latest date):
- If |t_long| >= SECTOR_T_LONG_STRONG: use LOOKBACK_LONG
- elif |t_mid| >= T_TH_SECTOR: use LOOKBACK_MID
- elif |t_short| >= T_TH_SECTOR: use LOOKBACK_SHORT (apply SHORT_WEIGHT_DISCOUNT to position sizing)
- else: no trade (sector gate fails)

Sector vol uses w_i = min(|raw_weight|, |weight_capped|) with original sign.
Constant intra-sector correlation rho (default 0.7).

Outputs:
- sector_gate includes t_short/t_mid/t_long and chosen lookback.
- positions includes inst_lookback and regime sizing multiplier.
...
def compute_hist_vol(prices: pd.DataFrame, window: int = 252, returns_method: str = "pct", annualize: bool = True) -> pd.DataFrame:
...
class V8Config:
    ...
    # Gate / filter thresholds
    T_TH_SECTOR: float = 1.0       # applied to mid/short gate
    SECTOR_T_LONG_STRONG: float = 1.5  # "very strong" long-trend threshold
    T_TH_INST: float = 1.0
    ...
    SHORT_WEIGHT_DISCOUNT: float = 0.5  # when using short lookback regime

    # Risk caps
    PER_INST_CAP: float = 0.05
    SECTOR_VOL_CAP: float = 0.15
...
def _choose_sector_regime(
    t_short: float,
    t_mid: float,
    t_long: float,
    cfg: V8Config,
) -> Tuple[int, int, float, float]:
    """Return (sector_dir, lookback_used, t_used, size_multiplier).

    sector_dir: +1 long, -1 short, 0 no-trade
    lookback_used: one of cfg LOOKBACK_*
    t_used: the t-stat used to set direction (for reporting)
    size_multiplier: applied to raw weight (e.g., discount for short regime)
    """
    # Prefer long regime when long-trend is very strong
    if np.isfinite(t_long) and abs(t_long) >= cfg.SECTOR_T_LONG_STRONG:
        return (1 if t_long > 0 else -1), cfg.LOOKBACK_LONG, t_long, 1.0

    # Otherwise use mid regime if it passes the normal gate
    if np.isfinite(t_mid) and abs(t_mid) >= cfg.T_TH_SECTOR:
        return (1 if t_mid > 0 else -1), cfg.LOOKBACK_MID, t_mid, 1.0

    # Otherwise allow short regime (but discount size) if it passes gate
    if np.isfinite(t_short) and abs(t_short) >= cfg.T_TH_SECTOR:
        return (1 if t_short > 0 else -1), cfg.LOOKBACK_SHORT, t_short, float(cfg.SHORT_WEIGHT_DISCOUNT)

    return 0, cfg.LOOKBACK_MID, np.nan, 0.0
...
def generate_v8(
    ...
    # Compute sector t-stats for all candidate lookbacks
    sec_t_short = compute_t_stat_nw(sector_indices, lookback=cfg.LOOKBACK_SHORT, lags=cfg.NW_LAGS)
    sec_t_mid = compute_t_stat_nw(sector_indices, lookback=cfg.LOOKBACK_MID, lags=cfg.NW_LAGS)
    sec_t_long = compute_t_stat_nw(sector_indices, lookback=cfg.LOOKBACK_LONG, lags=cfg.NW_LAGS)

    # Pick a latest date that exists in all three (most conservative)
    common_dates = set(sec_t_short.index) & set(sec_t_mid.index) & set(sec_t_long.index)
    if not common_dates:
        raise ValueError("No common dates across sector t-stat lookbacks. Check data length / NaNs.")
    latest_dt = max(common_dates)

    # Build sector regime table
    sectors = list(sector_indices.columns)
    rows = []
    sector_dir = {}
    sector_lb = {}
    sector_size_mult = {}
    ...
    for s in sectors:
        t_s = float(sec_t_short.loc[latest_dt].get(s, np.nan))
        t_m = float(sec_t_mid.loc[latest_dt].get(s, np.nan))
        t_l = float(sec_t_long.loc[latest_dt].get(s, np.nan))

        sdir, lb_used, t_used, mult = _choose_sector_regime(t_s, t_m, t_l, cfg)

        sector_dir[s] = int(sdir)
        sector_lb[s] = int(lb_used)
        sector_size_mult[s] = float(mult)
        sector_t_used[s] = float(t_used) if np.isfinite(t_used) else np.nan
        ...
        rows.append({
            ...
            "t_long": t_l,
            "sector_dir": int(sdir),
            "lookback_used": int(lb_used),
            "t_used": sector_t_used[s],
            "size_multiplier": float(mult),
            "gate_pass": (sdir != 0),
            "rule": "long->mid->short regime selection",
        })

    sector_gate_df = pd.DataFrame(rows)
    ...
    for sector, px in sector_to_prices.items():
        ...
        lb_used = int(sector_lb.get(sector, cfg.LOOKBACK_MID))
        mult = float(sector_size_mult.get(sector, 0.0))

        # For reporting, keep all three t's, plus the used one
        st_short = float(sector_gate_df.loc[sector_gate_df["sector"] == sector, "t_short"].iloc[0]) if sector in sector_dir else np.nan
        st_mid = float(sector_gate_df.loc[sector_gate_df["sector"] == sector, "t_mid"].iloc[0]) if sector in sector_dir else np.nan
        st_long = float(sector_gate_df.loc[sector_gate_df["sector"] == sector, "t_long"].iloc[0]) if sector in sector_dir else np.nan
        st_used = float(sector_gate_df.loc[sector_gate_df["sector"] == sector, "t_used"].iloc[0]) if sector in sector_dir else np.nan

        inst_t = compute_t_stat_nw(px, lookback=lb_used, lags=cfg.NW_LAGS)
        ...
        df_rank["lookback_used"] = lb_used
        df_rank["sector_t_used"] = st_used
        df_rank["sector_t_short"] = st_short
        df_rank["sector_t_mid"] = st_mid
        df_rank["sector_t_long"] = st_long
        df_rank["size_multiplier"] = mult
        ...
        raw_w = t_series * strength * (cfg.TARGET_VOL_ANNUAL_PER_INST / v_series) * mult
        ...
        # sector vol estimation uses min(|raw|, |cap|)
        ...
        for c in selected["wind_code"].tolist():
            positions_rows.append({
                "date": latest_dt,
                "sector": sector,
                "wind_code": c,
                "name": infer_name(describe, c),

                "sector_dir": sdir,
                "lookback_used": lb_used,
                "size_multiplier": mult,

                "sector_t_used": st_used,
                "sector_t_short": st_short,
                "sector_t_mid": st_mid,
                "sector_t_long": st_long,

                "inst_t": float(t_series.loc[c]),
                "vol_1y": float(v_series.loc[c]),
                "rho_intra_sector": float(cfg.RHO_INTRA_SECTOR),

                "strength": float(strength.loc[c]),
                "raw_weight": float(raw_w.loc[c]),
                "weight_capped": float(w_capped.loc[c]),
                "w_for_sector_vol": float(w_used.loc[c]),
                "sector_vol_before": float(sec_vol_before),
                "sector_scale": float(sec_scale),
                "sector_vol_after": float(sec_vol_after),
                "weight": float(w_final.loc[c]),
            })
...
def save_v8_excel(out_path: str, positions_df: pd.DataFrame, sector_gate_df: pd.DataFrame, rank_sheets: Dict[str, pd.DataFrame]) -> None:
...
if __name__ == "__main__":
    for asset_name in ['有色', '农产品', '化工', '黑色']:
        ...
        cfg = V8Config(
            LOOKBACK_SHORT=5,
            LOOKBACK_MID=26,
            LOOKBACK_LONG=126,  # 动态窗口集合（短/中/长）
            ...
            TARGET_VOL_ANNUAL_PER_INST=0.02,  # 单品种波动上限（决定 raw_weight 的“斜率”）
        ...
        positions, sector_gate, rank_sheets = generate_v8(
        ...
        # print("\n=== positions ===")
        # print(positions)
        out_path = r"CTA_" + asset_name + "_V8.xlsx"
        save_v8_excel(out_path, positions, sector_gate, rank_sheets)
