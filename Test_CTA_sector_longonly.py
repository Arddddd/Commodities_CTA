#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Long-only variant (top-2 equal weight) for chemical sector analysis.

Aligns factor calculations with Test_CTA_trend.py:
1) compute_vol_adj_mom
2) compute_t_stat_nw
3) compute_tanh_signal
"""
...
REPORT_PATH = "chem_sector_report_long_only.md"
...
def main() -> None:
    ...
    benchmark_col = "CIFI.WI" if "CIFI.WI" in prices else None
    ...
    sections: List[str] = []
    sections.append("# 化工板块趋势策略表现与鲁棒性（纯多头）\n")
    sections.append(f"数据源: `{DATA_PATH}` (日频更新数据)。\n")
    sections.append("\n## 1. 基准策略设定\n")
    sections.append(
        "- 资产池：化工板块内所有品种（剔除板块指数列 `CIFI.WI`）。\n"
        "- 因子 A：`compute_vol_adj_mom`，`lookback=5`，`vol_lookback=63`。\n"
        "- 因子 B：`compute_t_stat_nw`，`lookback=5`, `lags=2`。\n"
        "- 因子 C：`compute_tanh_signal`，`lookback=5`，`z_lookback=252`。\n"
        "- 权重：只做多板块内趋势最强的两个品种，权重各 1/2。\n"
